...
Normalmente, los sitios web comparten componentes de página comunes, como el 
encabezado, el pie de página, el menú y posiblemente muchos más. Estos 
componentes de página pueden usarse con el mismo diseño o con diseños 
diferentes. Existen dos estilos principales para organizar los diseños en los 
proyectos: el estilo *include* y el estilo *jerárquico*. Ambos estilos se pueden 
utilizar fácilmente con Thymeleaf sin perder su principal valor: 
**las plantillas naturales**.

### Diseños de estilo de inclusión

En este estilo, las páginas se crean integrando código de componente común 
directamente en cada vista para generar el resultado final. En Thymeleaf, esto 
se puede lograr mediante el **Sistema de Diseño Estándar de Thymeleaf**:
...
```
Los diseños de estilo de inclusión son bastante fáciles de entender e 
implementar y, de hecho, ofrecen flexibilidad para desarrollar vistas, lo cual 
constituye su mayor ventaja. Sin embargo, la principal desventaja de esta 
solución es que introduce cierta duplicación de código, por lo que modificar el 
diseño de un gran número de vistas en aplicaciones grandes puede resultar algo 
engorroso.
...
El sistema de diseño estándar de Thymeleaf ofrece la inclusión de fragmentos de 
página similares a las *inclusiones JSP*, con algunas mejoras importantes.
...
expresión después de los dos puntos es un selector de fragmentos (ya sea un nombre 
...
### Inclusión con selectores de marcado
...
sección en la documentación de Thymeleaf: [Sintaxis del Selector de Marcado](http://www.thymeleaf.org/doc/tutorials/3.1/usingthymeleaf.html#appendix-c-markup-selector-syntax).
...
código (por favor, tenga en cuenta que el valor de una variable puede ser una 
expresión):
...
Los fragmentos parametrizados permiten a los desarrolladores crear fragmentos 
funcionales que son más fáciles de reutilizar. Lea más sobre los fragmentos 
parametrizados en la documentación de Thymeleaf: 
...
De manera similar, podemos usar el mismo fragmento con otra clase breadcrumb en 
otra vista (`task/task.html`):
...
Si no hay nada que pasar al fragmento, podemos usar una expresión especial de 
fragmento vacío: `~{}`. Esta expresión pasará un valor vacío que se ignorará en 
el fragmento `header`:
...
Otra característica de la nueva expresión de fragmento es el token llamado 
*no-operation* que permite utilizar el marcado predeterminado del fragmento en 
...
La Expresión de Fragmentos permite personalizar fragmentos de maneras que hasta 
ahora solo eran posibles con el Dialecto de Diseño de terceros. Lea más sobre 
este tema en la documentación de Thymeleaf: 
[Diseños flexibles: más allá de la simple inserción de fragmentos](http://www.thymeleaf.org/doc/tutorials/3.1/usingthymeleaf.html#flexible-layouts-beyond-mere-fragment-insertion)
...
El fragmento anterior se carga cuando un nuevo usuario desea registrarse desde 
la página de inicio. El cuadro de diálogo modal se mostrará al hacer clic en el 
botón `Signup` y el contenido se cargará mediante una llamada AJAX 
(véase `home/homeNotSignedIn.html`).

### Referencias

Por favor, consulte la documentación de Thymeleaf que describe este tema detalladamente.

* [Diseño de plantilla](http://www.thymeleaf.org/doc/tutorials/3.1/usingthymeleaf.html#template-layout).
* [Expresiones de fragmentos](http://www.thymeleaf.org/doc/tutorials/3.1/usingthymeleaf.html#fragment-specification-syntax)
* [Diseños flexibles: más allá de la mera inserción de fragmentos](http://www.thymeleaf.org/doc/tutorials/3.1/usingthymeleaf.html#flexible-layouts-beyond-mere-fragment-insertion)
...
algunos atributos de Thymeleaf como `th:insert` o `th:replace`, la visualización 
...
diseño/decoración para diseñar/estilizar el contenido y permite pasar elementos de 
...
<html layout:decorate="~{task/layout}">
  ...
  <body>
    <!-- /* El contenido de esta página será decorado por los elementos de layout.html (task/layout) */ -->
    <div layout:fragment="content">
...
- Los controladores devuelven nombres de vista que se traducen a un único 
  archivo de vista Thymeleaf (1).
- Antes de renderizar la vista, el atributo `viewName` original en el objeto 
...
- La vista de diseño (2) contiene varios elementos incluidos:
`<div th:replace="${view} :: content">Page Content</div>`
- El archivo de vista propiamente dicho contiene fragmentos, *obtenidos* 
  mediante la plantilla, que incrusta la vista.
